...
void CHUD::Render()
{
	...
	if (CMario::GetInstance()->IsOnPowerMode())
		powerMeterAniId = ID_ANI_POWER_METER_TIER_7;
	else if (playerSpeedX < SPEED_X_TIER_1)
		powerMeterAniId = ID_ANI_POWER_METER_TIER_0;
	else if (playerSpeedX < SPEED_X_TIER_2)
		powerMeterAniId = ID_ANI_POWER_METER_TIER_1;
	else if (playerSpeedX < SPEED_X_TIER_3)
		powerMeterAniId = ID_ANI_POWER_METER_TIER_2;
	else if (playerSpeedX < SPEED_X_TIER_4)
		powerMeterAniId = ID_ANI_POWER_METER_TIER_3;
	else if (playerSpeedX < SPEED_X_TIER_5)
		powerMeterAniId = ID_ANI_POWER_METER_TIER_4;
	else if (playerSpeedX < SPEED_X_TIER_6)
		powerMeterAniId = ID_ANI_POWER_METER_TIER_5;
	else if (playerSpeedX < SPEED_X_TIER_7)
		powerMeterAniId = ID_ANI_POWER_METER_TIER_6;
	else
		powerMeterAniId = ID_ANI_POWER_METER_TIER_7;
	...
	CAnimations::GetInstance()->Get(powerMeterAniId)->Render(hudPosX - POWER_METER_POS_X, hudPosY - POWER_METER_POS_Y);
}
