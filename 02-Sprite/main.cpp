...
#define TEXTURE_PATH_MAP1 TEXTURES_DIR "\\map-1.png"
#define PATH_MAP1_TXT TEXTURES_DIR "\\map1.txt"



//LPCWSTR fileMAp = L"textures\\map1.txt";
CMario *mario;
...
#define ROW_TEXMAP 9
#define COL_TEXMAP 11
#define tileset_width 16
#define tileset_height 16
#define num_row_on_tilemap 41
#define num_col_on_tilemap 208
...
void ReadFile() {
	...
	ifstream fs(PATH_MAP1_TXT, ios::in);

	if (fs.fail())
	{
		fs.close();
		DebugOut(L"[ERROR] TileMap::Load_MapData failed: ID=%d", 10);
		return;
	}
...
void LoadResources()
{
	...
	LPTEXTURE texMap = textures->Get(ID_TEX_MAP1);

	int id_sprite = 1;
	
	for (UINT i = 0; i < ROW_TEXMAP; i++) {
		for (UINT j = 0; j < COL_TEXMAP; j++) {
